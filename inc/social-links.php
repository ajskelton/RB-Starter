...
function rb_social_links() {
	...
	foreach ( $socials as $social => $settings ) {
		...
		if ( ! empty( $url ) ) {
			$output[] = '<li><a href="' . esc_url_raw( $url ) . '" target="_blank" rel="noopener noreferrer">' . rb_icon( array( 'icon'  => $social,
			                                                                                                                     'group' => 'social',
			                                                                                                                     'label' => $settings['label']
				) ) . '<span class="label">' . esc_html( $settings['label'] ) . '</span></a></li>';
		}
	}
...
add_shortcode( 'social_links', 'rb_social_links' );
